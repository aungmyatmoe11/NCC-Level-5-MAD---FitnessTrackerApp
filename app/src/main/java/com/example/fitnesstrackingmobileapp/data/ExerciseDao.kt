...
interface ExerciseDao {
  ...
  @Insert(onConflict = OnConflictStrategy.IGNORE) suspend fun insert(note: Exercise)

  @Delete suspend fun delete(note: Exercise)

  @Query("Select * from exercise_table order by id ASC")
  fun getAllExercises(): LiveData<List<Exercise>>

  @Update suspend fun update(note: Exercise)

  //  @Query("SELECT SUM(calburned) AS total_calburned FROM notesTable")
  // suspend fun  totalCal():Double
  @Insert suspend fun insertLocation(locationData: LocationData)
